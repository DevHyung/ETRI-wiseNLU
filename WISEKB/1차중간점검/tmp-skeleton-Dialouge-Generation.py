...
def print_QA_by_index(_index) -> None:
    ...
    print("__________" * 3)
    print("입력발화     : {}".format(originQList[_index]))
...
def _replace_words(_qIdx, _iWords, _SqIdx, _dWords) -> str:
    ...
    #log('i', "Insertion Word:{}".format(_iWords))
    #log('i', "원래 답변 : {}".format(Sam))
    #log('i', "Deletion  Word:{}".format(_dWords))
...
def _extract_insertion_words_idx(_sentence) -> [int, Dict[str, List[int]]]:
    ...
    idx = __search_index(_sentence, originQList)
...
if __name__ == "__main__":
    ...
    # generate_response('미세먼지 때문에 창밖에 아무것도 안 보여.','다음 주가 벚꽃 절정이래.') # 21번 벚꽃을 O-> WT_TYPE 으로 바꿨음
    for i in range(len(QList)):
        try:
            generate_response(QList[i],SQList[i])  # 태그의 영향이 상당함
        except:
            exit(-1)
